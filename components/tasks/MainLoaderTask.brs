sub Init()
    ' set name of function in Task node component to be executed when the state field changes to RUN
    ' in our case this method executed after the following cmd: m.contentTask.control = "run" (see Init in MainScene)
    ...
    if json <> invalid
        for each category in json
            ...
            if Type(value) = "roArray" ' if parsed key value having other objects in it
                if category <> "series" ' ignore for this phase
                    row = {}
                    row.title = category
                    row.children = []
...
function GetItemData(video as Object) as Object
    ...
    if video.content <> invalid
        ' populate length of content to be displayed on teh GridScreen
